...
### BUG-001: Table shows "occupied" (yellow) without orders ✅ FIXED
...
- [x] Fix: Check if `total > 0` instead of just checking `session_id`
- [x] Auto-close empty sessions when user leaves TableOrderScreen

### BUG-002: Menu table missing click-to-add functionality ✅ FIXED
...
- [x] Add `on_data_table_row_selected()` to `TableOrderScreen` for menu-table

### BUG-003: Menu table missing zebra stripes ✅ FIXED
...
- [x] Add `zebra_stripes=True` to menu-table and order-table
